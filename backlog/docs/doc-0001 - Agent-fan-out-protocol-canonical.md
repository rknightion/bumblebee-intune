...
> authoritative for this repository — the checkout is self-contained, so an agent in CI or on
> another machine has the whole model without reaching outside it. Do not summarise, compress or
> adapt it. When the source changes, the edit is not finished until every consuming repo has been
> re-imported in the same change.
